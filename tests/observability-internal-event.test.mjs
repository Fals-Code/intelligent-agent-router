...
test("internal event schema rejects raw payload keys, secret identities, oversized collections and tampering", async () => {
  await assert.rejects(() => builder().create({ name: "9router.run.terminal", occurredAt: NOW, traceId: "trace-1", attributes: { "router.task": "raw task" } }), /sensitive\/raw payload data/);
...
test("projector maps runtime reconciliation without raw provider state or automatic redispatch claims", async () => {
  ...
  assert.equal(event.payload.name, "9router.runtime.reconciled"); assert.equal(event.payload.attributes["router.runtime.status"], "completed"); assert.equal(event.payload.attributes["router.runtime.automatic_redispatch_allowed"], false); assert.equal(event.payload.attributes["router.runtime.files_changed_count"], 1); assert.deepEqual(event.payload.links, [{ type: "runtime_session", reference: "runtime:opencode:ses-1" }]); assert.doesNotMatch(JSON.stringify(event), /raw\/workspace/); assert.doesNotMatch(JSON.stringify(event), /src\/a\.ts/);
});
