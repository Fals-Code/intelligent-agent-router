...
function sampleAuthorization({ assignment = "candidate", sampleId = "live-runtime-sample-1" } = {}) {
  ...
  return { schemaVersion: 1, algorithm: "sha256", authorizationId: `m5liveauth:${authorizationSha256.slice(0, 32).toLowerCase()}`, authorizationSha256, payload };
}

function experiment() {
  return { schemaVersion: 1, algorithm: "sha256", experimentId: "m5experiment:runtime-backed-deferred", experimentSha256: "A".repeat(64), payload: { projectId: "project-bounded-live-runtime", referenceSubjectId: "opencode:9router/hemat", candidateSubjectId: "opencode:9router/smart" } };
}

class RecordingRuntimeAdapter {
  constructor(runtimeId, sessionId, { failSend = false } = {}) {
    this.inner = new InMemoryAgentRuntimeAdapter({ runtimeId, createSessionId: () => sessionId, createEventId: (() => { let sequence = 0; return () => `${runtimeId}-event-${++sequence}`; })(), now: () => "2026-08-19T07:21:00.000Z" });
  ...
  async sendTask(sessionId, task) {
    this.tasks.push(structuredClone({ sessionId, task }));
...
function makeExecuteRun(machine, id, projectId) {
  ...
  assert.equal(run.phase, "execute"); assert.equal(run.status, "running"); return run;
}
...
async function fixture(t, { candidateFailSend = false } = {}) {
  ...
  const workflowStore = new JsonlWorkflowCheckpointStore({ filePath: join(root, "workflow.jsonl"), maxFileBytes: 512 * 1024, maxCheckpointBytes: 32 * 1024 });
  ...
  const shared = { prompt: "Return a concise bounded-live comparison result without tools or source changes.", context: ["R0 paired bounded-live execution.", "Output is not externally visible during dispatch."], toolIds: [] };
  const resolver = { async resolve({ role, subjectId }) { return role === "reference" ? { subjectId, run: referenceRun, workspace: root, adapter: referenceRuntime, bindingStore, task: { taskId: "deferred-live-reference-task", ...shared } } : { subjectId, run: candidateRun, workspace: root, adapter: candidateRuntime, bindingStore, task: { taskId: "deferred-live-candidate-task", ...shared } }; } };
...
test("runtime-backed deferred live dispatch durably binds two R0 zero-tool sessions before sending tasks and exposes no output", async (t) => {
  const ctx = await fixture(t); const authorization = sampleAuthorization();
  ...
  assert.equal(dispatch.payload.selectedRole, "candidate"); assert.equal(dispatch.payload.zeroRuntimeTools, true); assert.equal(dispatch.payload.candidateOutputExternallyVisible, false); assert.equal(dispatch.payload.automaticRedispatchAllowed, false); assert.equal(dispatch.payload.productionRoutingMutationAllowed, false);
  ...
  assert.equal(ctx.referenceRuntime.createRequests.length, 1); assert.equal(ctx.candidateRuntime.createRequests.length, 1); assert.equal(ctx.referenceRuntime.tasks.length, 1); assert.equal(ctx.candidateRuntime.tasks.length, 1);
  assert.equal(ctx.referenceRuntime.tasks[0].task.prompt, ctx.candidateRuntime.tasks[0].task.prompt); assert.deepEqual(ctx.referenceRuntime.tasks[0].task.context, ctx.candidateRuntime.tasks[0].task.context); assert.deepEqual(ctx.referenceRuntime.tasks[0].task.toolIds, []); assert.deepEqual(ctx.candidateRuntime.tasks[0].task.toolIds, []);
  ...
  assert.equal(reopened.get(ctx.referenceRun.id).sessionId, "ses_deferred_live_reference"); assert.equal(reopened.get(ctx.candidateRun.id).sessionId, "ses_deferred_live_candidate");
});

test("runtime-backed deferred live dispatch rejects tool/prompt drift before provider side effects", async (t) => {
  const ctx = await fixture(t); const authorization = sampleAuthorization();
  const toolResolver = { async resolve(input) { const base = await ctx.resolver.resolve(input); return input.role === "candidate" ? { ...base, task: { ...base.task, toolIds: ["read"] } } : base; } };
  await assert.rejects(() => new RuntimeBackedDeferredBoundedLiveExecutionCoordinator(toolResolver).dispatch({ experiment: experiment(), authorization }), /candidate runtime task must expose zero tools/);
  assert.equal(ctx.referenceRuntime.createRequests.length, 0); assert.equal(ctx.candidateRuntime.createRequests.length, 0);
  const driftResolver = { async resolve(input) { const base = await ctx.resolver.resolve(input); return input.role === "candidate" ? { ...base, task: { ...base.task, prompt: `${base.task.prompt} drift` } } : base; } };
  await assert.rejects(() => new RuntimeBackedDeferredBoundedLiveExecutionCoordinator(driftResolver).dispatch({ experiment: experiment(), authorization }), /identical prompt\/context\/tool policy/);
  assert.equal(ctx.referenceRuntime.createRequests.length, 0); assert.equal(ctx.candidateRuntime.createRequests.length, 0);
});

test("existing durable binding blocks deferred live redispatch before a second provider session", async (t) => {
  const ctx = await fixture(t); const authorization = sampleAuthorization(); await ctx.coordinator.dispatch({ experiment: experiment(), authorization });
  await assert.rejects(() => new RuntimeBackedDeferredBoundedLiveExecutionCoordinator(ctx.resolver).dispatch({ experiment: experiment(), authorization }), /already has durable binding; automatic redispatch is forbidden/);
  assert.equal(ctx.referenceRuntime.createRequests.length, 1); assert.equal(ctx.candidateRuntime.createRequests.length, 1);
});

test("candidate send failure after both durable bindings remains manual-reconciliation only", async (t) => {
  const ctx = await fixture(t, { candidateFailSend: true }); const authorization = sampleAuthorization();
  await assert.rejects(() => ctx.coordinator.dispatch({ experiment: experiment(), authorization }), /send side effect is uncertain after durable binding; manual reconciliation is required and automatic redispatch is forbidden/);
  assert.equal(ctx.referenceRuntime.createRequests.length, 1); assert.equal(ctx.candidateRuntime.createRequests.length, 1); assert.equal(ctx.referenceRuntime.tasks.length, 1); assert.equal(ctx.candidateRuntime.tasks.length, 1);
  assert.ok(ctx.bindingStore.get(ctx.referenceRun.id)); assert.ok(ctx.bindingStore.get(ctx.candidateRun.id));
  await assert.rejects(() => new RuntimeBackedDeferredBoundedLiveExecutionCoordinator(ctx.resolver).dispatch({ experiment: experiment(), authorization }), /already has durable binding; automatic redispatch is forbidden/);
});
