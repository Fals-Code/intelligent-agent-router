...
export interface LocalProductionRouterState {
  readonly schemaVersion: typeof LOCAL_PRODUCTION_ROUTER_STATE_SCHEMA_VERSION;
...
export interface LocalProductionRouterFingerprint {
  readonly schemaVersion: typeof LOCAL_PRODUCTION_ROUTER_FINGERPRINT_SCHEMA_VERSION;
  readonly algorithm: "sha256";
...
export interface LocalProductionRehearsalState {
  readonly schemaVersion: typeof LOCAL_PRODUCTION_REHEARSAL_STATE_SCHEMA_VERSION;
  readonly algorithm: "sha256";
  readonly stateId: string;
  readonly stateSha256: string;
...
interface PersistedRehearsalEntry {
  readonly schemaVersion: typeof LOCAL_PRODUCTION_REHEARSAL_JOURNAL_SCHEMA_VERSION;
...
export interface LocalProductionAdapterRehearsalReceipt {
  readonly schemaVersion: typeof LOCAL_PRODUCTION_REHEARSAL_RECEIPT_SCHEMA_VERSION;
  readonly algorithm: "sha256";
...
const REHEARSAL_PAYLOAD_FIELDS = new Set([
  ...
  "currentSubjectId", "routeRevision", "mutationCount", "updatedAt", "providerSpecificStatePersisted", "rawProviderOutputPersisted",
...
const EVENT_PAYLOAD_FIELDS = new Set([
  ...
  "readinessProposalId", "readinessProposalSha256", "targetSnapshotId", "targetSnapshotSha256", "sourceSnapshotId",
  "sourceSnapshotSha256", "adapterId", "adapterVersion", "adapterSourceSha256", "mainSourceSha256", "productionTargetId",
  "rehearsalTargetId", "projectId", "routeId", "capability", "beforeStateId", "beforeStateSha256", "afterStateId", "afterStateSha256",
  "beforeSubjectId", "afterSubjectId", "beforeRouteRevision", "afterRouteRevision", "observedAt", "recoveredAfterRestart", "reason",
  "productionRouteMutated", "automaticRetryAllowed", "automaticRollbackAllowed", "automaticRedispatchAllowed",
...
const RECEIPT_PAYLOAD_FIELDS = new Set([
  ...
  "adapterSourceSha256", "mainSourceSha256", "productionTargetId", "productionPreFingerprintId", "productionPreFingerprintSha256",
  "productionPostFingerprintId", "productionPostFingerprintSha256", "rehearsalTargetId", "candidateStateId", "candidateStateSha256",
  "restoredStateId", "restoredStateSha256", "candidateCommitEventId", "candidateCommitEventSha256", "restoreCommitEventId",
  "restoreCommitEventSha256", "candidateRecoveredAfterRestart", "restoreRecoveredAfterRestart", "completedAt", "classification",
  "productionRouteMutated", "automaticRetryAllowed", "automaticRollbackAllowed", "automaticRedispatchAllowed",
  "productionRoutingMutationAuthorized",
]);
...
export async function prepareLocalProductionRouterState(input: LocalProductionRouterStateInput): Promise<LocalProductionRouterState> {
  ...
  return Object.freeze({
    schemaVersion: LOCAL_PRODUCTION_ROUTER_STATE_SCHEMA_VERSION,
...
export class JsonFileLocalProductionReadOnlyTarget {
  ...
  constructor(input: { readonly descriptor: LocalProductionReadOnlyTargetDescriptor; readonly maxStateBytes: number }) {
    ...
    identity(input.descriptor.targetId, "production descriptor targetId");
  ...
  async fingerprint(observedAt: string): Promise<LocalProductionRouterFingerprint> {
    ...
    return Object.freeze({
      schemaVersion: LOCAL_PRODUCTION_ROUTER_FINGERPRINT_SCHEMA_VERSION,
      algorithm: "sha256" as const,
...
export class JsonFileLocalProductionRehearsalTarget {
  ...
  async writeCandidate(before: LocalProductionRehearsalState, candidateSubjectId: string, candidateRevision: string, updatedAt: string): Promise<LocalProductionRehearsalState> {
    ...
    if (current.stateId !== before.stateId || current.stateSha256 !== before.stateSha256) throw new Error("Rehearsal clone changed before candidate write");
    const next = await prepareRehearsalState({
      targetId: current.payload.targetId,
      sourceProductionTargetId: current.payload.sourceProductionTargetId,
      projectId: current.payload.projectId,
      routeId: current.payload.routeId,
      capability: current.payload.capability,
      currentSubjectId: identity(candidateSubjectId, "candidate subjectId"),
      routeRevision: identity(candidateRevision, "candidate routeRevision"),
  ...
  async restore(before: LocalProductionRehearsalState, referenceSubjectId: string, referenceRevision: string, updatedAt: string): Promise<LocalProductionRehearsalState> {
    const current = await this.read();
    if (current.stateId !== before.stateId || current.stateSha256 !== before.stateSha256) throw new Error("Rehearsal clone changed before restore write");
    const next = await prepareRehearsalState({
      targetId: current.payload.targetId,
      sourceProductionTargetId: current.payload.sourceProductionTargetId,
      projectId: current.payload.projectId,
      routeId: current.payload.routeId,
      capability: current.payload.capability,
      currentSubjectId: identity(referenceSubjectId, "restore subjectId"),
      routeRevision: identity(referenceRevision, "restore routeRevision"),
      mutationCount: current.payload.mutationCount + 1,
      updatedAt,
    });
    this.write(next);
  ...
  private write(state: LocalProductionRehearsalState): void {
    const dir = dirname(this.descriptor.stateFilePath);
    mkdirSync(dir, { recursive: true });
    const raw = `${JSON.stringify(state)}\n`;
    if (Buffer.byteLength(raw, "utf8") > this.maxStateBytes) throw new Error("Rehearsal clone state exceeds size limit");
    writeFileSync(this.descriptor.stateFilePath, raw, { encoding: "utf8", flag: "w" });
    const fd = openSync(this.descriptor.stateFilePath, "r");
...
export class JsonlLocalProductionRehearsalJournal {
  ...
  latest(operationId: string): LocalProductionRehearsalJournalEvent | undefined {
    for (let i = this.entries.length - 1; i >= 0; i -= 1) if (this.entries[i]?.event.payload.operationId === operationId) return this.entries[i]?.event;
    return undefined;
  }
  ...
  async assertFreshRead(): Promise<void> {
    ...
    const sha = await sha256Text(raw);
    if (size !== this.expectedFileSize || sha !== this.expectedFileSha256) throw new Error("Rehearsal journal durable state changed after reader opened");
  }
  ...
  async append(payload: LocalProductionRehearsalJournalPayload): Promise<LocalProductionRehearsalJournalEvent> {
    ...
    try { writeFileSync(fd, line, { encoding: "utf8" }); fsyncSync(fd); } finally { closeSync(fd); }
  ...
  private async load(): Promise<void> {
    ...
    if (Buffer.byteLength(raw, "utf8") > this.options.maxFileBytes) throw new Error("Rehearsal journal exceeds file size limit");
    if (raw.length > 0 && !raw.endsWith("\n")) throw new Error("Rehearsal journal is truncated or partial");
    const lines = raw.split("\n").filter(Boolean);
    const entries: PersistedRehearsalEntry[] = [];
    for (let i = 0; i < lines.length; i += 1) {
      const line = lines[i] ?? "";
      if (Buffer.byteLength(line, "utf8") > this.options.maxEventBytes) throw new Error("Rehearsal journal event exceeds size limit");
      const parsed = JSON.parse(line) as PersistedRehearsalEntry;
      if (parsed.schemaVersion !== 1 || parsed.sequence !== i + 1 || !parsed.event) throw new Error("Rehearsal journal sequence/envelope is invalid");
      await verifyJournalEvent(parsed.event);
      entries.push(parsed);
    }
    this.entries = entries;
    this.expectedFileSize = Buffer.byteLength(raw, "utf8");
...
export class LocalProductionAdapterRehearsalCoordinator {
  ...
  async applyCandidate(input: { readonly authority: LocalProductionAdapterRehearsalAuthority; readonly reservedAt: string; readonly appliedAt: string; readonly committedAt: string }): Promise<LocalProductionRehearsalJournalEvent> {
    ...
    const identityValue = operationIdentity(input.authority.readinessAuthorization.authorizationId, "candidate");
    if (this.journal.latest(identityValue.operationId)) throw new Error("Candidate rehearsal operation already exists; automatic retry is forbidden");
    ...
    const reservation = journalPayload("rehearsal_reserved", "candidate", identityValue, input.authority, this.productionTarget.descriptor.targetId, this.rehearsalTarget.descriptor.targetId, before, after, input.reservedAt, false, "candidate mutation reserved");
    ...
    return this.journal.append(journalPayload("rehearsal_committed", "candidate", identityValue, input.authority, this.productionTarget.descriptor.targetId, this.rehearsalTarget.descriptor.targetId, before, after, input.committedAt, false, "candidate mutation committed on rehearsal clone"));
  }
  ...
  async restoreReference(input: { readonly authority: LocalProductionAdapterRehearsalAuthority; readonly reservedAt: string; readonly restoredAt: string; readonly committedAt: string }): Promise<LocalProductionRehearsalJournalEvent> {
    await verifyRehearsalAuthority(input.authority);
    await assertProductionMatchesAuthority(this.productionTarget, input.authority);
    const identityValue = operationIdentity(input.authority.readinessAuthorization.authorizationId, "restore");
    if (this.journal.latest(identityValue.operationId)) throw new Error("Restore rehearsal operation already exists; automatic retry is forbidden");
    const before = await this.rehearsalTarget.read();
    ...
    await this.journal.append(journalPayload("rehearsal_reserved", "restore", identityValue, input.authority, this.productionTarget.descriptor.targetId, this.rehearsalTarget.descriptor.targetId, before, after, input.reservedAt, false, "reference restore reserved"));
    ...
    return this.journal.append(journalPayload("rehearsal_committed", "restore", identityValue, input.authority, this.productionTarget.descriptor.targetId, this.rehearsalTarget.descriptor.targetId, before, after, input.committedAt, false, "reference restore committed on rehearsal clone"));
  }
  ...
  async reconcile(input: { readonly authority: LocalProductionAdapterRehearsalAuthority; readonly phase: RehearsalPhase; readonly observedAt: string }): Promise<LocalProductionAdapterRehearsalRecoveryReport> {
    ...
    const identityValue = operationIdentity(input.authority.readinessAuthorization.authorizationId, input.phase);
    const event = this.journal.latest(identityValue.operationId);
    if (!event) return recoveryReport(input.phase, identityValue.operationId, "NO_OPERATION", input.observedAt, "No durable rehearsal operation exists", false);
    const current = await this.rehearsalTarget.read();
    if (event.payload.eventType === "rehearsal_committed") {
      if (current.stateId === event.payload.afterStateId && current.stateSha256 === event.payload.afterStateSha256) return recoveryReport(input.phase, identityValue.operationId, "COMMITTED", input.observedAt, "Durable commit matches current rehearsal state", false, event.eventId, current.stateId);
      await this.appendManual(input, identityValue, event, current, "Committed rehearsal state drifted from durable commit");
      return recoveryReport(input.phase, identityValue.operationId, "MANUAL_RECONCILIATION_REQUIRED", input.observedAt, "Committed rehearsal state drifted from durable commit", true, event.eventId, current.stateId);
    }
    if (event.payload.eventType === "rehearsal_manual_reconciliation_required") return recoveryReport(input.phase, identityValue.operationId, "MANUAL_RECONCILIATION_REQUIRED", input.observedAt, event.payload.reason, true, event.eventId, current.stateId);
    if (current.stateId === event.payload.beforeStateId && current.stateSha256 === event.payload.beforeStateSha256) return recoveryReport(input.phase, identityValue.operationId, "NOT_APPLIED_SAFE", input.observedAt, "Reservation exists and rehearsal clone remains at exact before state", true, event.eventId, current.stateId);
    if (current.stateId === event.payload.afterStateId && current.stateSha256 === event.payload.afterStateSha256) {
      ...
      return recoveryReport(input.phase, identityValue.operationId, "COMMITTED", input.observedAt, "Reservation reconciled to exact expected after state without duplicate write", false, commit.eventId, current.stateId);
    }
    await this.appendManual(input, identityValue, event, current, "Rehearsal clone is neither exact before nor exact expected after state");
    return recoveryReport(input.phase, identityValue.operationId, "MANUAL_RECONCILIATION_REQUIRED", input.observedAt, "Rehearsal clone is neither exact before nor exact expected after state", true, event.eventId, current.stateId);
  }
  ...
  async finalize(input: { readonly authority: LocalProductionAdapterRehearsalAuthority; readonly productionPreFingerprint: LocalProductionRouterFingerprint; readonly completedAt: string }): Promise<LocalProductionAdapterRehearsalReceipt> {
    await verifyRehearsalAuthority(input.authority);
    await this.journal.assertFreshRead();
    const candidate = this.journal.latestCommitted("candidate", input.authority.readinessAuthorization.authorizationId);
    const restore = this.journal.latestCommitted("restore", input.authority.readinessAuthorization.authorizationId);
    ...
    const receipt: LocalProductionAdapterRehearsalReceipt = Object.freeze({
      schemaVersion: LOCAL_PRODUCTION_REHEARSAL_RECEIPT_SCHEMA_VERSION,
      algorithm: "sha256" as const,
  ...
  private async appendManual(input: { readonly authority: LocalProductionAdapterRehearsalAuthority; readonly phase: RehearsalPhase; readonly observedAt: string }, identityValue: { operationId: string; idempotencyKey: string }, event: LocalProductionRehearsalJournalEvent, current: LocalProductionRehearsalState, reason: string): Promise<void> {
    await this.journal.append({
      ...event.payload,
      eventType: "rehearsal_manual_reconciliation_required",
      observedAt: timestamp(input.observedAt, "manual reconciliation observedAt"),
      recoveredAfterRestart: false,
      reason: `${reason}; current=${current.stateId}`,
      operationId: identityValue.operationId,
      idempotencyKey: identityValue.idempotencyKey,
    });
  }
}
...
export async function verifyLocalProductionAdapterRehearsalReceipt(
  ...
  if (!candidate || !restore || p.candidateCommitEventId !== candidate.eventId || p.candidateCommitEventSha256 !== candidate.eventSha256 || p.restoreCommitEventId !== restore.eventId || p.restoreCommitEventSha256 !== restore.eventSha256 || p.candidateStateId !== candidate.payload.afterStateId || p.candidateStateSha256 !== candidate.payload.afterStateSha256 || p.restoredStateId !== restore.payload.afterStateId || p.restoredStateSha256 !== restore.payload.afterStateSha256 || p.candidateRecoveredAfterRestart !== candidate.payload.recoveredAfterRestart || p.restoreRecoveredAfterRestart !== restore.payload.recoveredAfterRestart) throw new Error("Rehearsal receipt durable journal provenance drift detected");
...
async function expectedCandidateState(before: LocalProductionRehearsalState, authority: LocalProductionAdapterRehearsalAuthority, updatedAt: string): Promise<LocalProductionRehearsalState> {
  ...
  return prepareRehearsalState({
    ...
    sourceProductionTargetId: before.payload.sourceProductionTargetId,
    ...
    routeRevision: `rehearsal-candidate:${seed.slice(0, 32).toLowerCase()}`,
...
async function verifyRehearsalState(state: LocalProductionRehearsalState): Promise<void> {
  ...
  const p = state.payload as LocalProductionRehearsalStatePayload;
...
function validateEventPayload(p: LocalProductionRehearsalJournalPayload): void {
  assertExactFields(record(p, "Rehearsal journal payload"), EVENT_PAYLOAD_FIELDS, "Rehearsal journal payload");
  if (!["rehearsal_reserved", "rehearsal_committed", "rehearsal_manual_reconciliation_required"].includes(p.eventType) || !["candidate", "restore"].includes(p.phase)) throw new Error("Rehearsal journal event type/phase is invalid");
  const expectedIdentity = operationIdentity(p.readinessAuthorizationId, p.phase);
  if (p.operationId !== expectedIdentity.operationId || p.idempotencyKey !== expectedIdentity.idempotencyKey) throw new Error("Rehearsal journal operation/idempotency identity is not canonical");
  if (p.productionRouteMutated !== false || p.automaticRetryAllowed !== false || p.automaticRollbackAllowed !== false || p.automaticRedispatchAllowed !== false || p.productionRoutingMutationAuthorized !== false) throw new Error("Rehearsal journal safety authority is invalid");
  timestamp(p.observedAt, "rehearsal journal observedAt");
  text(p.reason, "rehearsal journal reason");
}
...
async function verifyJournalEvent(event: LocalProductionRehearsalJournalEvent): Promise<void> {
  ...
  validateEventPayload(event.payload as LocalProductionRehearsalJournalPayload);
...
function operationIdentity(authorizationId: string, phase: RehearsalPhase): { operationId: string; idempotencyKey: string } {
  const base = `local-production-rehearsal:${identity(authorizationId, "readiness authorizationId")}:${phase}`;
  return { operationId: base, idempotencyKey: base };
}
...
function recoveryReport(phase: RehearsalPhase, operationId: string, classification: LocalProductionAdapterRehearsalRecoveryClassification, observedAt: string, reason: string, explicitOperatorActionRequired: boolean, journalEventId?: string, rehearsalStateId?: string): LocalProductionAdapterRehearsalRecoveryReport {
  return Object.freeze({ phase, operationId, classification, journalEventId, rehearsalStateId, observedAt: timestamp(observedAt, "recovery observedAt"), reason, explicitOperatorActionRequired, automaticRetryAllowed: false, automaticRollbackAllowed: false, productionRoutingMutationAuthorized: false });
}
