...
/**
 * Durable control-plane ownership of workflow -> runtime session identity.
 * Provider-native session data is deliberately not persisted here.
 */
export class JsonlRuntimeBindingStore implements RuntimeBindingStore {
  ...
  bind(binding: RuntimeBinding): void {
    ...
    const line = `${JSON.stringify({
      schemaVersion: RUNTIME_BINDING_SCHEMA_VERSION,
    ...
    if (lineBytes > this.maxBindingBytes) {
      throw new Error(
        `Runtime binding exceeds maxBindingBytes: runId=${binding.workflowRunId} bytes=${lineBytes} max=${this.maxBindingBytes}`,
      );
    }
    if (this.expectedFileSize + lineBytes > this.maxFileBytes) {
      throw new Error(
        `Runtime binding append would exceed maxFileBytes: current=${this.expectedFileSize} append=${lineBytes} max=${this.maxFileBytes}`,
      );
    }
  ...
  private load(): void {
    ...
    if (size > this.maxFileBytes) {
      throw new Error(`Runtime binding file exceeds maxFileBytes: bytes=${size} max=${this.maxFileBytes}`);
    }
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      if (lineBytes > this.maxBindingBytes) {
        throw new Error(
          `Runtime binding at line ${lineNumber} exceeds maxBindingBytes: bytes=${lineBytes} max=${this.maxBindingBytes}`,
        );
      }
      const parsed = parseBinding(line, lineNumber);
      if (parsed.sequence !== expectedSequence) {
        throw new Error(
          `Runtime binding sequence mismatch at line ${lineNumber}: expected=${expectedSequence} actual=${parsed.sequence}`,
        );
      }
      expectedSequence += 1;
  ...
  private assertStorageUnchanged(): void {
    ...
    if (currentSize !== this.expectedFileSize) {
      throw new Error(
        `Runtime binding file changed outside this writer; reopen before binding: expectedBytes=${this.expectedFileSize} actualBytes=${currentSize}`,
      );
    }
  }
}
...
/**
 * Read-only restart reconciliation. It never resumes, retries, dispatches,
 * approves, publishes, or marks a workflow successful.
 */
export class RuntimeReconciliationCoordinator {
  async reconcile(
    run: WorkflowRun,
    binding: RuntimeBinding | undefined,
    probe: RuntimeReconciliationProbe | undefined,
    ...
    if (!binding) {
      return report(
        ...
        "manual_intervention",
        false,
        "Active workflow has no durable runtime binding; automatic re-dispatch is forbidden.",
      );
    }
    ...
    if (!probe) {
      return report(
        ...
        "manual_intervention",
        false,
        `No reconciliation probe is registered for runtime ${binding.runtimeId}.`,
      );
    }
    if (probe.runtimeId !== binding.runtimeId) {
      throw new Error(
        `Runtime reconciliation probe mismatch: binding=${binding.runtimeId} probe=${probe.runtimeId}`,
      );
    }
    ...
    try {
      ...
      return Object.freeze({
        ...report(
          ...
          "observation_failed",
          false,
          "Runtime observation failed; fail closed and require explicit intervention.",
        ),
    ...
    switch (observation.status) {
      ...
      case "waiting_approval":
        return report(
          ...
          "await_runtime_approval",
          false,
          "Runtime reports a pending approval; restart must not bypass it.",
        );
      case "completed":
        return report(
          run,
          ...
          "verify_runtime_result",
          true,
          "Runtime appears completed, but provider status is evidence only; deterministic verification is required before workflow advancement.",
        );
      case "interrupted":
        return report(
          run,
          ...
          "explicit_resume_or_retry",
          false,
          "Runtime was interrupted; resume or retry requires an explicit control-plane decision.",
        );
      case "failed":
      case "aborted":
      case "destroyed":
        return report(
          ...
          "explicit_failure_or_retry",
          false,
          `Runtime is ${observation.status}; workflow failure/retry remains an explicit control-plane transition.`,
        );
      case "created":
        return report(
          run,
          ...
          "manual_intervention",
          false,
          "Runtime session exists but no active/completed task can be proven; do not re-dispatch automatically.",
        );
    }
  }
}
...
function parseBinding(line: string, lineNumber: number): { sequence: number; binding: RuntimeBinding } {
  ...
  if (value.schemaVersion !== RUNTIME_BINDING_SCHEMA_VERSION) {
    throw new Error(`Unsupported runtime binding schema version at line ${lineNumber}: ${String(value.schemaVersion)}`);
  }
  if (!Number.isInteger(value.sequence) || Number(value.sequence) <= 0) {
    throw new Error(`Runtime binding sequence at line ${lineNumber} must be a positive integer`);
  }
...
function assertRuntimeBinding(value: unknown, label: string): asserts value is RuntimeBinding {
  ...
  if (!Number.isInteger(value.workflowAttempt) || Number(value.workflowAttempt) < 1) {
    throw new Error(`${label}.workflowAttempt must be a positive integer`);
  }
  if (!Number.isFinite(Date.parse(value.boundAt))) throw new Error(`${label}.boundAt must be a valid timestamp`);
}
...
function assertBindingProgression(previous: RuntimeBinding, next: RuntimeBinding): void {
  ...
  if (next.workflowAttempt <= previous.workflowAttempt) {
    throw new Error(`Runtime binding workflowAttempt must increase for ${next.workflowRunId}`);
  }
  if (Date.parse(next.boundAt) < Date.parse(previous.boundAt)) {
    throw new Error(`Runtime binding boundAt cannot move backwards for ${next.workflowRunId}`);
  }
}
...
function assertBindingMatchesWorkflow(binding: RuntimeBinding, run: WorkflowRun): void {
  ...
  if (binding.workflowAttempt !== run.attempt) {
    throw new Error(
      `Runtime binding attempt does not match workflow ${run.id}: binding=${binding.workflowAttempt} workflow=${run.attempt}`,
    );
  }
}
