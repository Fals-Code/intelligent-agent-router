...
import type { BoundedLivePublicationReceipt, VerifiedBoundedLiveRuntimeResult } from "./bounded-live-publication.js";
import { verifyBoundedLivePublicationReceipt, verifyVerifiedBoundedLiveRuntimeResultEnvelope } from "./bounded-live-publication.js";
import type { JsonlBoundedLiveSideEffectJournal } from "./bounded-live-side-effect-journal.js";
import { verifyBoundedLiveSideEffectEvent } from "./bounded-live-side-effect-journal.js";
...
export async function prepareVerifiedBoundedLivePairedExecution(input: {
  ...
  if (authorization.payload.experimentId !== experiment.experimentId || authorization.payload.experimentSha256 !== experiment.experimentSha256) throw new Error("Deferred bounded-live execution authorization does not match exact experiment");
  if (experiment.payload.referenceSubjectId === experiment.payload.candidateSubjectId) throw new Error("Deferred bounded-live execution requires distinct reference/candidate subjects");
  ...
  const reference = verifyRunBinding("reference", input.referenceRun, input.referenceBinding, experiment.payload.referenceSubjectId, input.referenceVerificationReference, experiment.payload.projectId);
  const candidate = verifyRunBinding("candidate", input.candidateRun, input.candidateBinding, experiment.payload.candidateSubjectId, input.candidateVerificationReference, experiment.payload.projectId);
  if (reference.runId === candidate.runId || reference.sessionId === candidate.sessionId) throw new Error("Deferred bounded-live execution requires distinct reference/candidate runs and sessions");
  if (normalizePath(input.referenceBinding.workspace) !== normalizePath(input.candidateBinding.workspace)) throw new Error("Deferred bounded-live reference/candidate runtimes must use the same workspace");
  const verifiedAt = prepareTimestamp(input.verifiedAt, "Deferred bounded-live paired verifiedAt");
  if (Date.parse(verifiedAt) < Date.parse(authorization.payload.approvedAt)) throw new Error("Deferred bounded-live paired verification cannot predate sample authorization");
  ...
  return deepFreeze({ schemaVersion: VERIFIED_BOUNDED_LIVE_PAIRED_EXECUTION_SCHEMA_VERSION, algorithm: "sha256" as const, executionId: `m5livepair:${executionSha256.slice(0, 32).toLowerCase()}`, executionSha256, payload });
}
...
export async function verifyVerifiedBoundedLivePairedExecutionEnvelope(execution: VerifiedBoundedLivePairedExecution): Promise<void> {
  if (!execution || typeof execution !== "object" || execution.schemaVersion !== VERIFIED_BOUNDED_LIVE_PAIRED_EXECUTION_SCHEMA_VERSION || execution.algorithm !== "sha256") throw new Error("Deferred bounded-live paired execution envelope is invalid");
  ...
  if (payload.bothRunLedgerOutcomesSucceeded !== true || payload.rawProviderOutputPersisted !== false || payload.candidateOutputExternallyVisibleBeforePublication !== false || payload.automaticRedispatchAllowed !== false || payload.productionRoutingMutationAllowed !== false) throw new Error("Deferred bounded-live paired execution safety flags are invalid");
  const expected = await sha256Canonical(payload);
  if (execution.executionSha256 !== expected || execution.executionId !== `m5livepair:${expected.slice(0, 32).toLowerCase()}`) throw new Error("Deferred bounded-live paired execution digest is invalid");
}
...
export class DeferredBoundedLiveExecutor {
  ...
  async reserve(input: { readonly experiment: ControlledExperimentDefinition; readonly authorization: BoundedLiveSampleAuthorization; readonly requestedAt: string }): Promise<ControlledExperimentExecutionJournalEvent> {
    await verifyBoundedLiveSampleAuthorizationEnvelope(input.authorization);
    const { experiment, authorization } = input;
    if (this.journal.experimentId !== experiment.experimentId || authorization.payload.experimentId !== experiment.experimentId || authorization.payload.experimentSha256 !== experiment.experimentSha256) throw new Error("Deferred bounded-live journal/authorization does not match exact experiment");
    ...
    if (this.sideEffects.inspect().unresolvedOperationIds.length > 0) throw new Error("Deferred bounded-live execution requires zero unresolved external side effects before reservation");
    if (state.completedShadowSamples !== authorization.payload.shadowSamplesBeforeLive || state.completedLiveSamples !== authorization.payload.liveSamplesBeforeDispatch || state.completedCandidateLiveSamples !== authorization.payload.candidateLiveSamplesBeforeDispatch) throw new Error("Deferred bounded-live authorization counters do not match durable completed journal state");
    if (state.reservedLiveSamples !== state.completedLiveSamples || state.reservedCandidateLiveSamples !== state.completedCandidateLiveSamples) throw new Error("Deferred bounded-live journal contains unresolved historical live reservations");
    return this.journal.reserveSample({ sampleId: authorization.payload.sampleId, exposure: "bounded_live", liveAssignment: authorization.payload.liveAssignment, inputReference: authorization.payload.inputReference, reservedAt: prepareTimestamp(input.requestedAt, "Deferred bounded-live requestedAt") });
  }
  ...
  async recordPublishedDispatch(input: {
    ...
    if (pair.sampleAuthorizationId !== authorization.authorizationId || pair.sampleAuthorizationSha256 !== authorization.authorizationSha256 || pair.sampleId !== authorization.payload.sampleId || pair.selectedRole !== authorization.payload.liveAssignment) throw new Error("Deferred bounded-live paired execution does not match exact sample authorization");
    if (selected.sampleAuthorizationId !== authorization.authorizationId || selected.sampleAuthorizationSha256 !== authorization.authorizationSha256 || selected.sampleId !== authorization.payload.sampleId || selected.role !== pair.selectedRole) throw new Error("Deferred bounded-live selected runtime result does not match pair/authorization");
    ...
    if (selected.runId !== expectedSelectedRunId || selected.sessionId !== expectedSelectedSessionId) throw new Error("Deferred bounded-live selected runtime result is not the selected member of paired execution");
    if (publication.sampleAuthorizationId !== authorization.authorizationId || publication.runtimeResultId !== input.selectedRuntimeResult.resultId || publication.sampleId !== authorization.payload.sampleId || publication.selectedRole !== pair.selectedRole) throw new Error("Deferred bounded-live publication receipt does not match selected verified execution");
    if (Date.parse(publication.publishedAt) < Date.parse(pair.verifiedAt)) throw new Error("Deferred bounded-live publication cannot predate paired deterministic verification");

    const sideEffect = this.sideEffects.latest(publication.sideEffectOperationId);
    if (!sideEffect) throw new Error("Deferred bounded-live publication has no durable side-effect operation");
    await verifyBoundedLiveSideEffectEvent(sideEffect);
    if (sideEffect.eventId !== publication.sideEffectCommitEventId || sideEffect.payload.eventType !== "operation_committed" || sideEffect.payload.kind !== "publication") throw new Error("Deferred bounded-live publication receipt is not backed by exact durable publication commit");
    if (sideEffect.payload.authorityId !== authorization.authorizationId || sideEffect.payload.subjectId !== authorization.payload.selectedSubjectId || sideEffect.payload.sampleId !== authorization.payload.sampleId || sideEffect.payload.outputSha256 !== publication.outputSha256 || sideEffect.payload.externalReference !== publication.publicationReference) throw new Error("Deferred bounded-live durable publication commit drifts from receipt/authorization");

    const latest = this.journal.latest(authorization.payload.sampleId);
    if (!latest || latest.payload.eventType !== "sample_reserved") throw new Error("Deferred bounded-live publication requires unresolved durable sample_reserved state");
    try {
      return await this.journal.recordDispatch({ sampleId: authorization.payload.sampleId, adapterId: publication.sinkId, acceptedAt: publication.publishedAt, referenceExecutionReference: pair.referenceExecutionReference, candidateExecutionReference: pair.candidateExecutionReference, candidateOutputExternallyVisible: publication.candidateOutputExternallyVisible });
    } catch (error) {
      throw new Error(`Deferred bounded-live publication is durably committed externally but sample_dispatched persistence failed; manual reconciliation is required and automatic retry/redispatch is forbidden: ${safeError(error)}`);
    }
  }

  async recordCompletion(input: { readonly authorization: BoundedLiveSampleAuthorization; readonly completedAt: string; readonly referenceObservationId: string; readonly candidateObservationId: string }): Promise<ControlledExperimentExecutionJournalEvent> {
    await verifyBoundedLiveSampleAuthorizationEnvelope(input.authorization);
    return this.journal.recordCompletion({ sampleId: input.authorization.payload.sampleId, completedAt: prepareTimestamp(input.completedAt, "Deferred bounded-live completedAt"), referenceObservationId: prepareIdentity(input.referenceObservationId, "Deferred bounded-live referenceObservationId"), candidateObservationId: prepareIdentity(input.candidateObservationId, "Deferred bounded-live candidateObservationId") });
  }
}
...
function verifyRunBinding(role: "reference" | "candidate", run: RunLedgerRecord, binding: RuntimeBinding, subjectId: string, verificationReferenceInput: string, projectId: string) {
  ...
  if (!run.evidence.some((item) => item.kind === "deterministic_check" && item.status === "passed" && item.reference === verificationReference)) throw new Error(`Deferred bounded-live ${role} lacks exact passed deterministic verification evidence`);
  return { runId: run.runId, sessionId: binding.sessionId, verificationReference };
}
function runtimeReference(role: "reference" | "candidate", binding: RuntimeBinding): string { return `bounded-live-runtime:${role}:${binding.runtimeId}:${binding.workflowRunId}:${binding.workflowAttempt}:${binding.sessionId}`; }
function normalizePath(value: string): string { return value.replace(/\\/g, "/").replace(/\/+$/, ""); }
function prepareIdentity(value: unknown, label: string): string { if (typeof value !== "string" || !value.trim()) throw new Error(`${label} must not be empty`); const prepared = value.trim(); if (/\r|\n/.test(prepared)) throw new Error(`${label} must be single-line`); return prepared; }
