...
export interface BoundedLiveRollbackAuthorizationInput {
...
export interface BoundedLiveRollbackAuthorizationPayload extends BoundedLiveRollbackAuthorizationInput {
  ...
  readonly rollbackWorkflowRunId: string;
  readonly projectId: string;
  readonly riskClass: "R3" | "R4";
  readonly strategy: "restore_reference_subject";
  readonly targetSubjectId: string;
  readonly guardrailClassification: "ROLLBACK_REQUIRED";
  readonly explicitReferenceRestoreAuthorized: true;
  readonly automaticRollbackAllowed: false;
  readonly generalProductionRoutingMutationAllowed: false;
}

export interface BoundedLiveRollbackAuthorization {
  readonly schemaVersion: typeof BOUNDED_LIVE_ROLLBACK_AUTHORIZATION_SCHEMA_VERSION;
  readonly algorithm: "sha256";
  readonly authorizationId: string;
  readonly authorizationSha256: string;
  readonly payload: BoundedLiveRollbackAuthorizationPayload;
}
...
export interface BoundedLiveReferenceRestoreReceiptPayload {
  readonly rollbackAuthorizationId: string;
  readonly rollbackAuthorizationSha256: string;
  readonly experimentId: string;
  readonly targetSubjectId: string;
  readonly sinkId: string;
  readonly restoreReference: string;
  ...
  readonly restoredAt: string;
  readonly activeSubjectId: string;
  readonly referenceSubjectRestored: true;
  readonly automaticRollbackAllowed: false;
  readonly automaticRetryAllowed: false;
  readonly generalProductionRoutingMutationAllowed: false;
}

export interface BoundedLiveReferenceRestoreReceipt {
  readonly schemaVersion: typeof BOUNDED_LIVE_REFERENCE_RESTORE_RECEIPT_SCHEMA_VERSION;
  readonly algorithm: "sha256";
  readonly receiptId: string;
  readonly receiptSha256: string;
  readonly payload: BoundedLiveReferenceRestoreReceiptPayload;
}
...
export async function prepareBoundedLiveRollbackAuthorization(input: {
  ...
  readonly rollbackWorkflow: WorkflowRun;
  readonly authorization: BoundedLiveRollbackAuthorizationInput;
  ...
  if (guardrailDecision.payload.classification !== "ROLLBACK_REQUIRED" || guardrailDecision.payload.guardrailActionRequired !== true) {
    throw new Error("Bounded-live rollback requires ROLLBACK_REQUIRED guardrail decision");
  }
  if (guardrailDecision.payload.automaticRollbackAllowed !== false || experiment.payload.rollback.automaticRollbackAllowed !== false) {
    throw new Error("Bounded-live rollback must remain explicitly authorized and non-automatic");
  }
  ...
  const payload: BoundedLiveRollbackAuthorizationPayload = deepFreeze({
    ...
    rollbackWorkflowRunId: rollbackWorkflow.id,
    ...
    targetSubjectId: experiment.payload.referenceSubjectId,
    guardrailClassification: "ROLLBACK_REQUIRED" as const,
    explicitReferenceRestoreAuthorized: true as const,
  ...
  return deepFreeze({
    schemaVersion: BOUNDED_LIVE_ROLLBACK_AUTHORIZATION_SCHEMA_VERSION,
...
function normalizeInput(input: BoundedLiveRollbackAuthorizationInput): BoundedLiveRollbackAuthorizationInput {
  return deepFreeze({
    actor: prepareIdentity(input.actor, "Bounded-live rollback actor"),
    approvedAt: prepareTimestamp(input.approvedAt, "Bounded-live rollback approvedAt"),
    policyReferences: normalizeSafeSet(input.policyReferences, "Bounded-live rollback policy reference", true),
