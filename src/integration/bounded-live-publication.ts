...
export async function prepareVerifiedBoundedLiveRuntimeResult(
  input: VerifiedBoundedLiveRuntimeResultInput,
): Promise<VerifiedBoundedLiveRuntimeResult> {
  const authorization = input.authorization;
  if (input.role !== authorization.payload.liveAssignment) {
    throw new Error("Bounded-live runtime result role does not match sample authorization liveAssignment");
  }
  ...
  if (input.run.runId !== input.binding.workflowRunId
    || input.run.projectId !== input.binding.projectId
    || input.run.runtimeId !== input.binding.runtimeId) {
    throw new Error("Bounded-live runtime result Run Ledger identity does not match durable RuntimeBinding");
  }
  if (input.run.projectId !== authorization.payload.projectId) {
    throw new Error("Bounded-live runtime result projectId does not match sample authorization");
  }
  if (!Number.isInteger(input.binding.workflowAttempt) || input.binding.workflowAttempt <= 0) {
    throw new Error("Bounded-live RuntimeBinding workflowAttempt is invalid");
  }
  ...
  const verificationEvidence = input.run.evidence.find((item) =>
    item.kind === "deterministic_check" && item.status === "passed" && item.reference === verificationReference,
  );
...
export async function verifyVerifiedBoundedLiveRuntimeResult(
  result: VerifiedBoundedLiveRuntimeResult,
  input: VerifiedBoundedLiveRuntimeResultInput,
): Promise<void> {
  const expected = await prepareVerifiedBoundedLiveRuntimeResult(input);
  if (result.schemaVersion !== VERIFIED_BOUNDED_LIVE_RUNTIME_RESULT_SCHEMA_VERSION || result.algorithm !== "sha256") {
    throw new Error("Bounded-live runtime result envelope is invalid");
  }
  if (result.resultId !== expected.resultId || result.resultSha256 !== expected.resultSha256
    || stableStringify(result.payload) !== stableStringify(expected.payload)) {
...
export class BoundedLivePublicationCoordinator {
  ...
  async publish(input: {
    readonly authorization: BoundedLiveSampleAuthorization;
    readonly runtimeResult: VerifiedBoundedLiveRuntimeResult;
  }): Promise<BoundedLivePublicationReceipt> {
    ...
    if (runtimeResult.payload.runLedgerOutcome !== "succeeded" || runtimeResult.payload.rawOutputPersisted !== false) {
      throw new Error("Bounded-live publication requires succeeded verified result without persisted raw output");
    }
    if (authorization.payload.automaticDispatchAllowed !== false
      || authorization.payload.automaticRedispatchAllowed !== false
      || authorization.payload.productionRoutingMutationAllowed !== false
    ...
    const output = await this.reader.read({
      runtimeId: runtimeResult.payload.runtimeId,
      sessionId: runtimeResult.payload.sessionId,
      runId: runtimeResult.payload.runId,
    });
    ...
    return deepFreeze({
      schemaVersion: BOUNDED_LIVE_PUBLICATION_RECEIPT_SCHEMA_VERSION,
      algorithm: "sha256" as const,
...
export async function verifyBoundedLivePublicationReceipt(receipt: BoundedLivePublicationReceipt): Promise<void> {
  if (!receipt || typeof receipt !== "object") throw new Error("Bounded-live publication receipt must be an object");
  if (receipt.schemaVersion !== BOUNDED_LIVE_PUBLICATION_RECEIPT_SCHEMA_VERSION || receipt.algorithm !== "sha256") {
    throw new Error("Bounded-live publication receipt envelope is invalid");
  }
  const expected = await sha256Canonical(receipt.payload);
  if (receipt.receiptSha256 !== expected || receipt.receiptId !== `m5livepub:${expected.slice(0, 32).toLowerCase()}`) {
    throw new Error("Bounded-live publication receipt digest is invalid");
  }
  if (receipt.payload.externallyVisible !== true
    || receipt.payload.rawOutputPersisted !== false
    || receipt.payload.automaticRetryAllowed !== false
    || receipt.payload.automaticRollbackAllowed !== false
    || receipt.payload.productionRoutingMutationAllowed !== false) {
    throw new Error("Bounded-live publication receipt safety flags are invalid");
  }
  if (receipt.payload.candidateOutputExternallyVisible !== (receipt.payload.selectedRole === "candidate")) {
    throw new Error("Bounded-live candidate visibility flag does not match selected role");
  }
}

function assertSinkReceipt(
  receipt: Awaited<ReturnType<BoundedLivePublicationSink["publish"]>>,
  sinkId: string,
  idempotencyKey: string,
  role: BoundedLiveAssignment,
  outputSha256: string,
): void {
  if (!receipt || typeof receipt !== "object") throw new Error("Bounded-live publication sink returned invalid receipt");
  if (receipt.sinkId !== sinkId || receipt.idempotencyKey !== idempotencyKey || receipt.selectedRole !== role
    || receipt.outputSha256 !== outputSha256 || receipt.externallyVisible !== true) {
